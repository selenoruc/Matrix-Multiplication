...
#include <omp.h>        //OpenMP library added for parallelizm

type** create_matrix(){
    type** created_matrix = new type*[SIZE];
    #pragma omp parallel for    //OpenMP parallelism
...
void matrix_multiplication(type** matrix_A, type** matrix_B, type** matrix_C){
    ...
    cout << endl << SIZE << " x " << SIZE << " parallel matrix multiplication calculating ...\n";
    start = clock();
    
    #pragma omp parallel for    //OpenMP parallelism //Pragma won't affect g++ compiler
    for(int i=0; i<SIZE; i++){
        for(int j=0; j<SIZE; j++){
